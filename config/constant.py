    ...
    DB_TO_SQLALCHEMY_TYPE_MAPPING = (
        {
            ...
            'int4': 'Integer',          # integer 别名
            'int8': 'BigInteger',       # bigint 别名
            'float4': 'Float',          # real 别名
            'float8': 'Float',          # double precision 别名
    ...
    DB_TO_PYTHON_TYPE_MAPPING = (
        {
            ...
            'double': 'float',          # double 类型 (list_column 返回的类型)
            'float': 'float',           # float 类型
            'bool': 'bool',             # boolean 别名
            'timestamptz': 'datetime',  # timestamp with time zone 别名
            'timetz': 'time',           # time with time zone 别名
            'USER-DEFINED': 'str',      # PostgreSQL 枚举类型 (映射为 str)
        }
        if DataBaseConfig.db_type == 'postgresql'
        else {
            # 数值类型
