...
schema_modelsController = APIRouter(prefix='/schema_model/schema_models',
...
async def get_schema_model_schema_models_list(
    ...
    # 获取分页数据
    schema_models_page_query_result = await Schema_modelsService.get_schema_models_list_services(query_db,
                                                                                                 schema_models_page_query,
                                                                                                 is_page=True)
    logger.info('获取成功')
...
async def add_schema_model_schema_model_with_root(
    ...
    create_schema_model_result = await Schema_modelsService.add_schema_model_with_root_services(query_db,
                                                                                                create_schema_model)
...
@schema_modelsController.post(
    '/generate',
    dependencies=[Depends(CheckUserInterfaceAuth('schema_model:schema_models:add'))],
    summary='生成JSON Schema 数据模型',
    description='根据 JSON 或 JSON Schema 生成可视化节点；未规划来源返回明确开发中提示',
)
async def generate_schema_model_schema_models(
        request: Request,
        generate_schema_model: SchemaModelGenerateRequestModel,
):
    logger.info(generate_schema_model.model_dump())
    generate_result = Schema_modelsService.generate_schema_model_services(generate_schema_model)
    logger.info(generate_result.message)

    return ResponseUtil.success(msg=generate_result.message, data=generate_result.model_dump(by_alias=True))


@schema_modelsController.post(
    '/copy',
    dependencies=[Depends(CheckUserInterfaceAuth('schema_model:schema_models:add'))],
    summary='复制JSON Schema 数据模型',
    description='复制模型主信息与所有可视化节点，可指定目标目录',
)
async def copy_schema_model_schema_models(
        request: Request,
        copy_schema_model: SchemaModelCopyRequestModel,
        query_db: AsyncSession = Depends(get_db),
        current_user: CurrentUserModel = Depends(LoginService.get_current_user),
):
    logger.info(copy_schema_model.model_dump())
    copy_result = await Schema_modelsService.copy_schema_model_services(
        query_db,
        copy_schema_model,
    ...
    return ResponseUtil.success(msg=copy_result.message, data=copy_result.result)


@schema_modelsController.post(
    '/copy-to-branch',
    dependencies=[Depends(CheckUserInterfaceAuth('schema_model:schema_models:add'))],
    summary='复制JSON Schema 数据模型到其他分支',
    description='复制模型主信息与所有可视化节点，并写入目标分支ID',
)
async def copy_schema_model_to_branch_schema_models(
        request: Request,
        copy_schema_model: SchemaModelCopyRequestModel,
        query_db: AsyncSession = Depends(get_db),
        current_user: CurrentUserModel = Depends(LoginService.get_current_user),
...
@schema_modelsController.post(
    ...
    summary='移动JSON Schema 数据模型到目录',
    description='更新模型所属目录并递增版本号',
)
async def move_schema_model_schema_models(
        request: Request,
        move_schema_model: SchemaModelMoveRequestModel,
        query_db: AsyncSession = Depends(get_db),
        current_user: CurrentUserModel = Depends(LoginService.get_current_user),
):
    logger.info(move_schema_model.model_dump())
    move_result = await Schema_modelsService.move_schema_model_services(
        query_db,
        move_schema_model,
        operator=current_user.user.user_name,
    )
    ...
    return ResponseUtil.success(msg=move_result.message, data=move_result.result)
...
async def delete_schema_model_schema_models(request: Request, model_ids: str, query_db: AsyncSession = Depends(get_db)):
    ...
    delete_schema_models_result = await Schema_modelsService.delete_schema_models_services(query_db,
                                                                                           delete_schema_models)
...
async def query_detail_schema_model_schema_models(request: Request, model_id: str,
                                                  query_db: AsyncSession = Depends(get_db)):
...
async def export_schema_model_schema_models_list(
    ...
    # 获取全量数据
    schema_models_query_result = await Schema_modelsService.get_schema_models_list_services(query_db,
                                                                                            schema_models_page_query,
                                                                                            is_page=False)
    schema_models_export_result = await Schema_modelsService.export_schema_models_list_services(
        schema_models_query_result)
    logger.info('导出成功')
