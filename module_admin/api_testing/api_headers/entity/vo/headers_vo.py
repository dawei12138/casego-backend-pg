...
class HeadersModel(BaseModel):
    ...
    header_id: Optional[int] = Field(default=None, description='ID')
    case_id: Optional[int] = Field(default=None, description='关联的测试用例ID')
    key: Optional[str] = Field(default=None, description='请求头键名')
    value: Optional[str] = Field(default=None, description='请求头值')
    is_run: Optional[int] = Field(default=None, description='是否启用该请求头')
    ...
    @NotBlank(field_name='key', message='请求头键名不能为空')
    def get_key(self):
        return self.key
...
class HeadersQueryModel(HeadersModel):
    """
    接口请求头不分页查询模型
    """
    pass
...
@as_query
class HeadersPageQueryModel(HeadersQueryModel):
    """
    接口请求头分页查询模型
    """
...
class DeleteHeadersModel(BaseModel):
    """
    删除接口请求头模型
    """
    ...
    header_ids: str = Field(description='需要删除的ID')
