...
class SysOperLog(Base):
    ...
    method = Column(Text, nullable=True, default='', comment='方法名称')
    ...
    oper_url = Column(Text, nullable=True, default='', comment='请求URL')
    ...
    oper_param = Column(Text, nullable=True, default='', comment='请求参数')
    json_result = Column(Text, nullable=True, default='', comment='返回参数')
    status = Column(Integer, default=0, comment='操作状态（0正常 1异常）')
    error_msg = Column(Text, nullable=True, default='', comment='错误消息')
