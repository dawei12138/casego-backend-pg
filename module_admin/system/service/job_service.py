...
from module_admin.system.entity.vo.job_vo import DeleteJobModel, EditJobModel, JobModel, JobPageQueryModel
...
class JobService:
        ...
        if not CronUtil.validate_cron_expression(page_object.cron_expression):
            ...
            try:
                ...
                if job_info.status == '0':
                    await SchedulerUtil.add_scheduler_job(job_info=job_info)
    ...
    async def edit_job_services(cls, query_db: AsyncSession, page_object: EditJobModel):
        ...
        if job_info:
            ...
            try:
                ...
                # 先移除旧任务
                await SchedulerUtil.remove_scheduler_job(job_id=edit_job.get('job_id'))
                # 重新查询完整的任务信息（包含更新后的所有字段）
                job_info = await cls.job_detail_services(query_db, edit_job.get('job_id'))
                # 根据最新的 status 判断是否需要添加到调度器
                if job_info.status == '0':
                    await SchedulerUtil.add_scheduler_job(job_info=job_info)
                await query_db.commit()
    ...
    async def execute_job_once_services(cls, query_db: AsyncSession, page_object: JobModel):
        ...
        :return: 执行一次定时任务结果
        """
        await SchedulerUtil.remove_scheduler_job(job_id=page_object.job_id)
        job_info = await cls.job_detail_services(query_db, page_object.job_id)
        if job_info:
            await SchedulerUtil.execute_scheduler_job_once(job_info=job_info)
        ...
        if page_object.job_ids:
            ...
            try:
                for job_id in job_id_list:
                    ...
                    await SchedulerUtil.remove_scheduler_job(job_id=job_id)
                await query_db.commit()
