...
from module_admin.system.dao.login_dao import login_by_account
from module_admin.system.dao.user_dao import UserDao
from module_admin.system.entity.do.menu_do import SysMenu
from module_admin.system.entity.vo.common_vo import CrudResponseModel
from module_admin.system.entity.vo.login_vo import MenuTreeModel, MetaModel, RouterModel, SmsCode, UserLogin, UserRegister
from module_admin.system.entity.vo.user_vo import AddUserModel, CurrentUserModel, ResetUserModel, TokenData, UserInfoModel
...
class LoginService:
    ...
    async def get_current_user(
        ...
        # token_parse_time = time.time() - token_parse_start
        # logger.warning(f"Token解析耗时: {token_parse_time:.3f}s")

        # 2. 数据库查询用户信息阶段
        # db_query_start = time.time()
        query_user = await UserDao.get_user_by_id(query_db, user_id=token_data.user_id)
        # db_query_time = time.time() - db_query_start
        # logger.warning(f"数据库查询用户信息耗时: {db_query_time:.3f}s")
        ...
        # redis_verify_time = time.time() - redis_verify_start
        # logger.warning(f"Redis验证耗时: {redis_verify_time:.3f}s")
        ...
        if token == redis_token:
            ...
            # logger.warning(f"Redis更新耗时: {redis_update_time:.3f}s")

            # 5. 权限处理阶段
            # permission_process_start = time.time()
            ...
            # permission_process_time = time.time() - permission_process_start
            # logger.warning(f"权限处理耗时: {permission_process_time:.3f}s")

            # 6. 用户对象构建阶段
            # object_build_start = time.time()
            ...
            # object_build_time = time.time() - object_build_start
            # logger.warning(f"用户对象构建耗时: {object_build_time:.3f}s")

            # # 总耗时统计
            # total_time = time.time() - start_time
            # logger.warning(f"登录状态验证总耗时: {total_time:.3f}s")
            # logger.warning(f"各阶段耗时占比 - Token解析:{token_parse_time / total_time * 100:.1f}%, "
            #                f"数据库查询:{db_query_time / total_time * 100:.1f}%, "
            #                f"Redis验证:{redis_verify_time / total_time * 100:.1f}%, "
            #                f"Redis更新:{redis_update_time / total_time * 100:.1f}%, "
            #                f"权限处理:{permission_process_time / total_time * 100:.1f}%, "
            #                f"对象构建:{object_build_time / total_time * 100:.1f}%")
    ...
    #         logger.warning(f"登录状态验证耗时: {acquire_time:.3f}s")
    #         return current_user
    #     else:
    #         logger.warning('用户token已失效，请重新登录')
    #         raise AuthException(data='', message='用户token已失效，请重新登录')
