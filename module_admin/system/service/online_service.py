...
from module_admin.system.entity.vo.online_vo import DeleteOnlineModel, OnlineQueryModel
    ...
    async def get_online_list_services(cls, request: Request, query_object: OnlineQueryModel):
        ...
        for item in access_token_values_list:
            ...
            online_dict = dict(
                ...
                ipaddr=payload.get('login_info').get('ipaddr') if payload.get('login_info') else None,
                login_location=payload.get('login_info').get('loginLocation') if payload.get('login_info') else None,
                browser=payload.get('login_info').get('browser') if payload.get('login_info') else None,
                os=payload.get('login_info').get('os') if payload.get('login_info') else None,
                login_time=payload.get('login_info').get('loginTime') if payload.get('login_info') else None,
            )
